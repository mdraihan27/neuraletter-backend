...
@router.get("/user")
def get_all_topics_by_user(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
@router.get("/{topic_id}")
def get_topic_by_id(topic_id:str, current_user:dict = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
def delete_topic_by_id(topic_id:str, current_user:dict = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
def update_topic_by_id(topic_id:str, topic_update:TopicUpdate, current_user:dict = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
